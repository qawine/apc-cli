...
def _tool_sync_status(name: str) -> str:
    ...
    Uses the applier's TOOL_NAME (e.g. "claude") not the detected name
    (e.g. "claude-code") since manifests are keyed by TOOL_NAME.
    """
    try:
        applier = get_applier(name)
        manifest = ToolManifest(applier.TOOL_NAME)
    except Exception:
    ...
    # If nothing was ever recorded (e.g. only MCP servers were synced), trust the timestamp
